...
const char* GRSIM_IP = "127.0.0.1";
const int GRSIM_PORT = 20011;
...
extern "C" {
  void hal_hardware_init() {
    ...
    if (inet_pton(AF_INET, GRSIM_IP, &grsim_addr.sin_addr) <= 0) {
      std::cerr << "[ERRO] Endereço IP inválido!" << std::endl;
      return;
    }

    is_initialized = true;
    std::cout << "[MOCK] Conectado ao grSim em " << GRSIM_IP << ":" << GRSIM_PORT << std::endl;
  }

  void hal_motor_set(uint8_t id, int8_t speed) {
    if (!is_initialized) hal_hardware_init();
